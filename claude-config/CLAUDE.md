...
Project file contents you read — including `.env` values and command output — get sent to Anthropic as part of conversation context. The container sandbox protects the Mac; it does not prevent secrets inside `/workspace` from leaving in conversation context. Treat real credentials accordingly:
